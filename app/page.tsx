    ...
    <div className="space-y-16">
      ...
      <section className="pt-12">
        ...
        <p className="mt-6 max-w-2xl text-lg text-muted">
          {cases.length} casos verificables · {patterns.length} patrones
          recurrentes · {frameworks.length} frameworks teóricos comparados.
      ...
      <section>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <ModuleCard
            ...
            badge="En vivo"
            title="Pattern Atlas"
            desc="Mapa mundial interactivo + filtros por tier. Visualiza convergencia de fenómenos a través de 79 años."
          />
          <ModuleCard
            badge="Roadmap"
            title="Bayesian Lab"
            desc="Calculadora interactiva. Ajusta tus priors sobre las 9 hipótesis y compara con el análisis del corpus."
            soon
          />
          <ModuleCard
            badge="Roadmap"
            title="Convergence Engine"
            desc="Buscador semántico. Selecciona un caso y encuentra los estructuralmente más similares en el corpus."
            soon
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((c) => (
            <FeaturedCase key={c.id} caseData={c} />
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mt-3 max-w-2xl text-muted">
          El aporte analítico central. Cualquier evento UAP institucional cae
          en una de cuatro categorías según quién controla el contenido vs
            ...
            <tbody>
              <tr className="border-t border-border">
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-accent">8m</span> Cover-up clásico
                  <br />
                  <span className="text-xs text-muted">“no hay nada que ver” · 1947-2016</span>
                </td>
                <td className="px-4 py-3">
                  <span className="font-mono text-accent">8r</span> Leaks/WikiLeaks
                  <br />
              ...
              <tr className="border-t border-border">
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-accent">8n</span> Ambigüedad estratégica
                  <br />
                  <span className="text-xs text-muted">PURSUE 2026</span>
                </td>
                <td className="px-4 py-3">
                  <span className="font-mono text-accent">8q</span> Ecosystem disclosure
                  <br />
...
function ModuleCard({
  ...
  const inner = (
    <div
      className={`rounded-lg border p-5 transition ${
        soon
          ? "border-border bg-panel opacity-70"
          : "group border-border bg-panel hover:border-accent/50"
      }`}
    >
      <div className="flex items-center gap-2">
        <span className="rounded bg-bg px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest text-muted">
          {badge}
        </span>
        {soon && (
          <span className="rounded bg-tierA/20 px-2 py-0.5 font-mono text-[10px] uppercase text-tierA">
            próximamente
          </span>
        )}
      </div>
      <h3
        className={`mt-3 text-lg font-medium ${
          soon ? "text-muted" : "text-text group-hover:text-accent"
        }`}
      >
        {title}
      </h3>
  ...
  return soon || !href ? inner : <a href={href}>{inner}</a>;
}

function FeaturedCase({ caseData }: { caseData: import("@/lib/types").UAPCase }) {
  const tierClasses =
    caseData.tier === "S"
      ? "border-tierS/40 bg-tierS/5"
      : caseData.tier === "A"
        ? "border-tierA/40 bg-tierA/5"
        : "border-tierB/40 bg-tierB/5";
  return (
    <a
      href={`/cases/${caseData.id}`}
      className={`block rounded-lg border ${tierClasses} p-4 transition hover:border-accent/50`}
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-medium text-text">
          {caseData.flag} {caseData.name}
        </p>
