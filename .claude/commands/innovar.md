...
**INNOVACIÓN de UX/UI/visual** (elevar *cómo se ve, se navega e interactúa* lo que ya existe — categoría de primera clase, no un extra):
- **Visual/UI** — ancla en una **señal visual concreta** (la regla anti-fluff aplica aquí con más fuerza): un componente que renderiza datos en tabla/texto plano donde un chart contaría mejor la historia; una interacción que vive en una vista y falta en su hermana (ej. el hover sincronizado segmento↔leyenda del `MeceDonut` existe en `/probabilidades` pero no donde se repite el patrón); un campo del schema sin tratamiento visual propio (`posterior` MECE, `tier`, país×década); un token de `tailwind.config.ts` infrautilizado; un WARN de `audit-design` que además es oportunidad de rediseño, no solo de parche. *"Mejorar la UX" sigue prohibido*; *"la home lista las 6 narrativas MECE en texto pero el `MeceDonut` interactivo ya existe — reusarlo cierra la inconsistencia"* está permitido.
- **UX/flujo** — ancla en una **fricción o hueco de navegación concreto**, no en "reducir fricción" a secas: una referencia cruzada en prosa que debería ser link navegable (los casos del ciclo 2026 se citan como "caso 138" en texto plano, sin `<a>`); una ruta huérfana no enlazada desde nav/footer/sitemap; una combinación de filtros que deja un estado vacío confuso sin salida; un flujo que termina en callejón (el fallback "⏳ Caso pendiente" de casos delgados); un touch target bajo el mínimo AA que rompe una tarea en móvil (`audit-design`). Verifica el grafo de navegación real (`app/*/page.tsx`, `components/MobileNav.tsx`, el footer del layout, `app/sitemap.ts`) antes de afirmar que algo es huérfano o inalcanzable.
- Respeta el **ADN de UX/UI** del repo (además de las restricciones de §3):
  - **Server-components-first + enhancement progresivo**: el SSR debe renderizar la vista COMPLETA y la interactividad se añade encima (patrón `MeceDonut`: SSR deja el donut entero, el hover/tap es progresivo). Nada que quede en blanco sin JS.
  - **Tokens editoriales** (`bg/panel/border/text/muted/accent/tierS/A/B` en `tailwind.config.ts`): diseña dentro de la paleta crema/tinta/acento, no metas colores sueltos; usa el lookup estático `TIER_META` de `lib/ui.ts` (la JIT no compila `bg-${x}`).
  - **Bilingüe**: todo texto nuevo va en `<T es en>` — labels de charts y SVG incluidos.
  - Para cualquier chart/visualización nueva, **carga antes el skill `dataviz`** (paleta, tipos de marca, accesibilidad) — no improvises colores ni ejes.
- Antes de proponer un COMPONENTE nuevo, revisa `components/` + `components/explorer/` para no reinventarlo (mismo anti-pattern que casi duplica `/cobertura`).
