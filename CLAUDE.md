...
- **Deuda pendiente (jul 2026) · el redondeo desmiente la leyenda «suma 100%»**: `pct()` de `components/MeceChart.tsx` hace `toFixed(0)` **por categoría, de forma independiente**, y el pie afirma «suma 100%» / «sums to 100%». Con la partición al 50,5/23/19/7/3 la barra por caso imprime **101%** en pantalla. El fix es *largest remainder* (truncar todo hacia abajo y repartir las unidades sobrantes a los mayores restos decimales); aplica a `CasePosterior` y a las vistas derivadas de `MecePartition` (líneas de prosaico/anómalo). **Ojo, el test NO lo detecta**: `scripts/test-chart.mjs` valida los valores **crudos** (suma=1, arcos = circunferencia), no los porcentajes ya redondeados, así que pasa en verde con el bug presente — al arreglarlo, sumar un assert sobre el texto renderizado.
...
- **Deuda pendiente (jul 2026) · el scraper anónimo sigue entrando**: el tope de 15 lo saca de las **métricas**, no del **sitio**, y `robots.txt` no lo toca porque no se declara (ráfagas desde datacenters de Singapur; 28 páginas de una sola IP el 19 jul = más que los 19 lectores humanos de ese día juntos). Bloquearlo de verdad exige una regla en el **firewall de Vercel** (Deny por ASN es más quirúrgico que un rate limit y no toca lectores reales) — se identifica en *Firewall → Traffic* filtrando por los picos (29-30 jun, 9-10 jul, 17-18 jul), que es además el **único sitio donde se ven los user-agents**: Supabase no los guarda por diseño de privacidad.
- **`public.app_content` NO es de este proyecto** (hallazgo jul 2026): el proyecto Supabase `uap-codex` está **compartido con otra app** — esa tabla guarda el HTML de un «English Trainer» (2 filas, 14 KB + 67 KB), sin una sola referencia en este repo. **No borrarla ni asumirla huérfana** (se escribió el 19 jul 2026). Tiene **RLS deshabilitada**, así que cualquiera con la anon key —que viaja en el bundle del navegador— puede **leerla y escribirla**. El riesgo real es la escritura, no la lectura: el fix seguro es RLS + política de `select` público y **sin** políticas de escritura, pero antes hay que confirmar si esa app escribe con la anon key (entonces la rompe) o vía dashboard/`service_role` (a quien RLS no le aplica).
...
- **No** tomar el título de un video DVIDS como metadata oficial del Departamento de Guerra sin leer el `og:description` completo (meta tag, no el bloque visible "VIDEO INFO") — puede describir una cadena de custodia radicalmente distinta a la que el nombre del archivo sugiere. Los 50 videos de **PURSUE Release 02** llevan, sin excepción confirmada, el mismo banner: "AARO identified a collection of responsive materials held on a classified network... Many of these materials lack a substantiated chain-of-custody", y cada título es 'uploader-defined' (lo asignó quien subió el archivo, no el DoW) mientras la AARO solo "assesses... likely" el origen. Releases 01/04/05 en cambio sí documentan una cadena institucional trazable ("[CENTCOM/NORTHCOM/...] submitted a report to AARO"). Costó 4 casos con framing de "título oficial del DoW" que hubo que reescribir (`tyndall-uscg-tictac-2024`, `three-fast-moving-uap-2020`, `spherical-afghanistan-2020`, `spherical-uap-quad-clip-2021`), más correcciones en `pursue-release-2026` y `lake-huron-2023` (ago 2026). Ver [`docs/dvids-inventario.md`](docs/dvids-inventario.md).
- **No** concluir que una entrega de PURSUE "no publicó video" (o cualquier ausencia similar) a partir de una búsqueda DVIDS con un solo patrón de query — un "0 resultados" puede ser el límite del término buscado, no un hallazgo real. El Release 03 (12 jun 2026) se registró como "0 videos" durante toda la sesión porque las búsquedas usaban `q=DOW-UAP` y el tag `AARO061226`, ambos correctos para las otras 4 entregas; sus 6 videos existían todo el tiempo bajo una serie con prefijo distinto, `FBI-UAP-PR*` (no `DOW-UAP-PR*`), invisible a esas queries. Apareció recién al buscar `q=FBI-UAP` — y ese giro solo ocurrió porque el usuario cuestionó el "0" en vez de darlo por bueno. Ante cualquier "no existe"/"0 resultados" de una fuente externa con más de un prefijo o convención de nombres posible, probar al menos una query con un término alternativo antes de reportarlo como ausencia real. *(ago 2026, ver `docs/dvids-inventario.md`)*
- **No** aceptar solicitudes de enlace saliente a explainers secundarios de terceros, por bien presentadas que lleguen, y **declinar por el motivo estructural, nunca por un vacío puntual del artículo** — la razón es que el corpus cita fuentes primarias y su página de caso ya contiene aquello por lo que el lector iría al explainer, así que el enlace no le aporta nada: el único que gana es el dominio de destino. Ese motivo no caduca. El del vacío puntual sí, y ahí está la trampa: en sep 2026 se declinó el explainer de Insider Release sobre Immaculate Constellation porque omitía el wargame Schriever y a Colavito —la evidencia que bajó el caso de 55 a 25 y cambió su `mundanoType` a misid—, y la carta lo encuadró como "una razón editorial específica, no una política general". **Cerraron el vacío**: agregaron una sección que nombra a Colavito y a Metabunk y trata la lectura del wargame como hipótesis competidora seria, y volvieron a pedir el enlace, legítimamente. Dos lecciones: (1) encuadrar un rechazo como un vacío específico invita a cerrarlo y re-pedir, así que el motivo declarado debe ser el duradero desde la primera carta; (2) **una regla cuya justificación escrita es un hecho perecedero se lee como void cuando el hecho cambia** —esta misma entrada afirmaba una omisión que ya no existe—, de modo que la razón estructural va primero y el ejemplo queda como ilustración fechada. Reconocer el mérito ajeno cuando corresponde no cuesta nada y hace terminal la negativa; el patrón del outreach (halago investigado, posicionamiento como publicación par "document-first", pedido envuelto en "a quiet further-reading link" / "no reciprocal ask", firma "Backlinks / Partnerships") se reconoce, pero no es lo que decide. *(sep 2026)*
...
- **Un issue PERSISTENTE, no uno por corrida** (ago 2026). El workflow busca el issue abierto por su marcador HTML (`<!-- link-rot-gate -->`), lo actualiza si existe y solo abre uno nuevo si no; cuando el gate vuelve a verde, lo comenta y lo cierra. Antes abría uno por corrida: se acumularon **82 issues automáticas abiertas** y el gate pasó a ser ruido de fondo — el mismo fracaso que la línea base buscaba evitar («un check rojo permanente se ignora»), reintroducido por la capa de notificación en vez de por la de medición. Mismo patrón en el gate de DVIDS (`<!-- dvids-embed-gate -->`). **Lección general: al construir un guardrail, la política de notificación es parte del guardrail** — una sonda correcta que grita todos los días acaba silenciada igual que una incorrecta.
- **Cadencia semanal, no diaria** (ago 2026, `daily-audit.yml` → `weekly-audit.yml`). El mismo antipatrón de la viñeta anterior reapareció un nivel más arriba: aun con el issue consolidado, el *run* del workflow seguía marcándose rojo y disparando un email por GitHub cada vez que aparecía cualquier cita nueva rota — casi a diario, dado el volumen de citas del corpus. El fix de fondo no es silenciar el fallo, es preguntar si la cadencia calza con la velocidad real del fenómeno: el link rot se acumula en escala de semanas, no de horas, así que revisar a diario no compraba nada — solo generaba ruido más seguido. Cron pasó a semanal (lunes 07:00 UTC); el issue persistente sigue siendo la única superficie de seguimiento.

Corre en **`weekly-audit.yml`**, no en el prebuild: necesita red, y las sondas de build son node-plain offline a propósito. Abre issue con label `audit`/`automated` al detectar roturas nuevas.
...
**Registro de búsqueda (jun 2026, ampliado jul 2026) — NO re-buscar**: el barrido completo de Commons (28 verificados y aplicados en A–E, 3 en la sección F, el resto confirmado sin retrato libre, cola agotada en ambos) vive en [`docs/registros.md`](docs/registros.md#fotos-de-actores--barrido-de-commons). **No re-buscar sin leerlo** — incluye una pista de Library of Congress (foto de los Hill) ya descartada por copyright activo, y el caso Philip Spencer, negativo estructural porque es un seudónimo sin nombre real publicado.
