...
- **Inglés es el idioma PRIMARIO del sitio** (SEO global, desde jul 2026): la raíz `/` sirve inglés (`<html lang="en">`, default del CSS bilingüe = EN, metadata/canonical en inglés, `x-default` hreflang = raíz inglesa) y el **espejo español vive en `/es`**. El corpus es bilingüe en el DOM (`<T es en>` + CSS por `data-locale`); `app/es/layout.tsx` fija `data-locale="es"` server-side. (Antes era español-primario; se invirtió por alcance internacional.)
  ...
  - **Deuda que QUEDA · chrome bilingüe (~56 spans/página)**: el header/footer (`app/layout.tsx`, `FooterLink`) y `MobileNav`/`SiteSearch` siguen emitiendo los dos idiomas — son **compartidos por rutas espejo Y no-espejo**, así que un idioma-por-URL ahí exige que el chrome sepa el locale por `usePathname` (client) o extraer el footer a un client component; se dejó para un pase aparte porque el grueso del texto (cuerpos) ya se capturó. No es una fuga de cuerpo: el chrome se sincroniza al idioma de la URL vía `data-locale`.
