...
- **No** alimentar la metadata de una ruta **inglesa** con `seoTitle`/`seoDescription`: esos campos están en **español** y son la variante ES (`data/cases/*.json`). La raíz `/cases/[slug]/` es la ruta inglesa y los usaba directamente, cayendo a `name_en` solo cuando faltaban — así que 15 casos servían un `<title>` en español a la SERP anglófona. Coste medido en Search Console (11 jun–9 jul): **542 impresiones y 0 clicks** en AAWSAP + Grusch, ambas en **posición ~7**, con EE.UU. aportando el 60% de las impresiones a **CTR 0,3%** (vs 7–14% donde el idioma coincide). Desde jul 2026 la ruta EN usa solo `seoTitle_en`/`seoDescription_en`, con fallback a `name_en`/`summary_en`. Es el mismo error que la regla de arriba (campo con par bilingüe consumido crudo), pero en `generateMetadata` en vez del render — **la metadata también tiene pares `_en`**.
  - **Deuda pendiente (SEO, jul 2026) · `seoTitle` ES huérfano**: `/es/cases/[slug]/` pasa `c.name`/`c.summary` a `esMeta()` y **nunca lee `seoTitle`/`seoDescription`**, así que los títulos ES optimizados no se sirven en ninguna parte. Y donde `name` está en inglés (`grusch-testimony-2023` → "Grusch Congressional testimony", `robertson-panel-1953` → "Robertson Panel") la ruta **española** muestra título en inglés. El fix exige que `esMeta` soporte título **absoluto** (hoy siempre aplica el template `· UAP Codex`, y `seoTitle` está pensado sin sufijo, ≤60 chars) — helper compartido por muchas páginas, así que no encadenar a otro cambio. Impacto bajo: el público ES es ~5% de las impresiones. Solo **3 de los 15** casos tienen `seoTitle_en` a medida (los que concentran 571 de 576 impresiones); los otros 12 caen a `name_en`, que es correcto pero no está optimizado por query.
