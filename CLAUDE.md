...
Es **una de cuatro superficies de disparo** —CLI, panel, formulario de `workflow_dispatch` (`.github/workflows/cerebro.yml`) y atajo del teléfono contra la API de GitHub—, todas lanzando el mismo `claude -p "/cerebro <modo>"`. El recorrido de cada una, el scope exacto del PAT y **por qué el botón no puede vivir en un artifact** (no hay conector de GitHub en claude.ai + la CSP bloquea hosts externos; registro fechado para no re-investigarlo) están en [`docs/disparar-el-cerebro.md`](docs/disparar-el-cerebro.md).
