...
- **No** importar el corpus completo (`cases`/`patterns`/… desde `lib/data`) en un módulo alcanzable desde un componente cliente —directa o transitivamente—, aunque solo expongas agregados (porque webpack lo embute en el chunk cliente: `MobileNav` del layout global importaba `lib/siteStats`, que derivaba `STATS` del corpus, embutiendo los ~4 MB de `cases.json` en CADA página — el LCP killer real, 5,23→0,92 MB de JS al arreglarlo, jul 2026). Precalcula los agregados a un JSON diminuto en build (`data/site-stats.json` vía `build-cases.mjs`) e impórtalo en su lugar. Al diagnosticar bundle bloat, traza el grafo de módulos desde los `"use client"` — no adivines el componente (los de `/atlas` y `/laboratorio` eran inocentes; el culpable estaba en el layout). **Corolario (jul 2026): incluso `cases-client.json` (el bundle sin prosa, ~1.5 MB) es demasiado para una viz que solo usa unos campos.** `WorldMap` (/atlas) importaba `casesClient` entero para pintar marcadores que solo necesitan `id/name/tier/country/country_name/year/probability/location` → proyecta un dataset a-medida (`data/atlas-points.json` vía `build-cases.mjs`, **68 KB**, `lib/atlasData.ts`), no el corpus-menos-prosa. El slim de prosa protege el LCP global; el slim de campos protege la ruta que lo consume. *(enforzado parcialmente: `audit-consistency.mjs` E18d blinda la superficie del explorer —`lib/explorer/**`, `components/explorer/**`— contra imports de `lib/data`/`lib/meceModel`, y exige que `lib/meceClasses.ts` siga data-free. La regla transitiva general NO se mecaniza: las fronteras de `dynamic()` —chunks lazy legítimos como `/atlas`, `/laboratorio`— la harían frágil, así que ahí sigue siendo doc + criterio.)*
- **No** re-exportar una const (`export { X } from "./y"`) y seguir usándola localmente en el mismo módulo sin importarla también — el re-export NO crea binding local, así que `X` queda «Cannot find name» y rompe el build (pasó con `MUNDANO_SUBTYPES` al moverlo a `lib/meceClasses` y re-exportarlo desde `meceModel`, que lo usa en `expandedHypotheses`, #638; el patrón correcto —re-export **+** `import { X }`— ya vivía en `MECE_CLASSES`, se olvidó replicar). Ojo con la verificación: el `tsc` local sin `node_modules` da **falso verde** en estos errores de resolución (degrada; solo el `next build` de CI los ve) — para atraparlos local, filtra los `Cannot find name` **reales** del ruido de `react`/`next`/`node` ausente, no confíes en «0 errores».
  - **Falso verde aún peor (jul 2026): `tsc <archivos>` desde `web/` NO CHEQUEA NADA.** Con un `tsconfig.json` presente y archivos en la línea de comandos, tsc aborta con `error TS5112` («tsconfig.json is present but will not be loaded if files are specified on commandline») y sale — el filtro de ruido se traga esa única línea y queda «0 errores» sobre código que ni se parseó. Se detectó porque un **control negativo** (romper un cierre `});` a propósito) tampoco disparó. Usa `tsc --noEmit --ignoreConfig --jsx preserve --target esnext --moduleResolution bundler --skipLibCheck <archivos>` y **valida siempre la sonda con un control negativo antes de creerle** — es la misma lección de «verifica la sonda contra un caso cuyo resultado ya conoces», aplicada a la herramienta que verifica.
...
- **No** declarar `alternates.languages` sin verificar que el espejo EXISTE, ni omitirlo cuando existe — el `hreflang` solo vale si es **recíproco**, y apuntando a un 404 invalida el cluster entero. Las páginas `/es` pasan siempre por `esMeta` (emite el par por construcción), así que la mitad que driftea es **siempre la raíz inglesa**, donde el `languages` se escribe a mano. Ambas direcciones se vieron en el deploy vivo (jul 2026): `/releases/01..04/` con **cero** tags mientras `/es/releases/NN/` sí las apuntaba (anotación unidireccional → Google la ignora, 8 páginas), y `/calidad/` declarando `es: /es/calidad/` con `app/es/calidad/` **inexistente** (404 confirmado en `gh-pages`). E23 vigila `MIRRORED` ↔ directorios `/es` y E27 el sitemap; **ninguna miraba la metadata**, por eso la clase era invisible. *(enforzado: `audit-consistency.mjs` E31, ERROR, en ambas direcciones e incluyendo rutas dinámicas — el drift ocurrió justo en una)*
- **No** escribir la metadata de una sección a mano (`export const metadata = { title, description, alternates }`) — usa `pageMeta`. Next **no hace deep-merge de `openGraph`**, así que la ruta hereda el del layout y su card social apunta a la **home**: en el deploy vivo `/atlas/`, `/fuentes/`, `/contact/` y `/visitantes/` emitían `og:url = https://uapcodex.org/` con el copy genérico del sitio (jul 2026). El leak entra en silencio porque `pageMeta` es opt-in y una ruta nueva se escribe a mano. Esa **misma superficie a mano** es la que se saltó la revisión de idioma: las 5 rutas raíz no espejadas servían `<title>` en **español** en el sitio inglés-primario —mismo costo medido que la regla del `seoTitle` de arriba— frente a las espejadas, que sirven inglés. *(enforzado: `audit-consistency.mjs` E32, ERROR; excepciones legítimas la home —el `openGraph` del layout ya se auto-referencia en `/`—, las gated y las noindex. El **idioma** de la metadata NO se mecaniza: una heurística ES/EN sobre títulos daría falsos positivos con nombres propios; queda como criterio en la revisión que E32 fuerza.)*
